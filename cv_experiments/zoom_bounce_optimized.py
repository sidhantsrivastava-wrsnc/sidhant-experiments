...
def _rgb_to_nv12(rgb, buf_nv12):
    """Convert RGB uint8 (h,w,3) to NV12 in-place into pre-allocated buffer.

    NV12 layout: h rows of Y (luma), then h/2 rows of interleaved UV (chroma).
    Total size: h*1.5 rows × w columns, dtype uint8.
    """
    h, w, _ = rgb.shape
    ...
    buf_nv12[:h, :] = y.astype(np.uint8)

    # Subsample chroma 2×2
    r2 = (r[0::2, 0::2] + r[0::2, 1::2] + r[1::2, 0::2] + r[1::2, 1::2]) * 0.25
    g2 = (g[0::2, 0::2] + g[0::2, 1::2] + g[1::2, 0::2] + g[1::2, 1::2]) * 0.25
    b2 = (b[0::2, 0::2] + b[0::2, 1::2] + b[1::2, 0::2] + b[1::2, 1::2]) * 0.25

    u = (-0.148 * r2 - 0.291 * g2 + 0.439 * b2 + 128.0)
    v = (0.439 * r2 - 0.368 * g2 - 0.071 * b2 + 128.0)
    np.clip(u, 0, 255, out=u)
    np.clip(v, 0, 255, out=v)

    # Interleave U,V into NV12 chroma plane
    uv_plane = buf_nv12[h:, :]
    uv_plane[:, 0::2] = u.astype(np.uint8)
    uv_plane[:, 1::2] = v.astype(np.uint8)
...
def apply_zoom_blur(
    ...
    buf_warped_f32=None, buf_blend=None,
):
    """
    ...
    if _HAS_CUPY:
        ...
        _apply_zoom_blur_cpu(buf_warped, rgb, M, w, h, strength, n_samples,
                             buf_accum, buf_sample, buf_warped_f32, buf_blend)


def _apply_zoom_blur_cpu(buf_warped, rgb, M, w, h, strength, n_samples,
                         buf_accum, buf_sample, buf_warped_f32=None,
                         buf_blend=None):
    cx, cy = w / 2.0, h / 2.0
    base_zoom = M[0, 0]
    ...
    for i in range(n_samples):
        ...
        np.add(buf_accum, buf_sample, out=buf_accum, casting='unsafe')

    buf_accum /= n_samples
    if buf_warped_f32 is not None:
        np.copyto(buf_warped_f32, buf_warped, casting='unsafe')
        orig_f = buf_warped_f32
    else:
        orig_f = buf_warped.astype(np.float32)
    if buf_blend is not None:
        np.subtract(buf_accum, orig_f, out=buf_blend)
        np.multiply(buf_blend, strength, out=buf_blend)
        np.add(orig_f, buf_blend, out=buf_blend)
        np.clip(buf_blend, 0, 255, out=buf_blend)
        np.copyto(buf_warped, buf_blend, casting='unsafe')
    else:
...
class _ThreadedDecoder:
    ...
    decode of frame N+1 overlaps processing of frame N.
    """
...
def _render_active_segment(
    ...
    # Pre-allocate overlay ROI buffer (overlay image size is fixed)
    buf_ovl_roi = None
    if overlay:
        oi_sample, _ = overlay.get_frame(0)
        oh, ow_ = oi_sample.shape[:2]
        ...
        if overlay and overlay_config and p > 0.01:
            ...
            if opacity > 0:
                ...
                if x1 < x2 and y1 < y2:
                    s1, s2 = x1 - ox, y1 - oy
                    rh, rw = y2 - y1, x2 - x1
                    np.copyto(buf_ovl_roi[:rh, :rw], buf_out[y1:y2, x1:x2], casting='unsafe')
                    roi = buf_ovl_roi[:rh, :rw]
                    o = oi[s2:s2 + rh, s1:s1 + rw]
                    a = om[s2:s2 + rh, s1:s1 + rw] * opacity
